...
export default function AdminNutritionPage() {
  ...
  const loadTemplates = async () => {
    try {
      ...
      const data: NutritionTemplate[] = snapshot.docs.map((docItem) => ({
        id: docItem.id,
        ...(docItem.data() as Omit<NutritionTemplate, "id">),
      }));

      setTemplates(data);
    } catch (error) {
      console.error("Load nutrition templates error:", error);
    }
  };
  ...
  const createTemplate = async () => {
    ...
    try {
      ...
      await loadTemplates();
      alert("Nutrition template created.");
    } catch (error) {
      console.error("Create nutrition template error:", error);
      alert("Failed to create nutrition template.");
    } finally {
      setSaving(false);
    }
  };
  ...
  const deleteTemplate = async (id: string) => {
    const confirmed = window.confirm("Delete this nutrition template?");
    if (!confirmed) return;
    ...
    try {
      ...
      console.error("Delete nutrition template error:", error);
  ...
  return (
    <main className="min-h-screen bg-white p-10">
      ...
      <h1 className="mt-6 text-3xl font-bold">Nutrition Templates</h1>
      <p className="mt-2 text-gray-600">
        Create reusable menus and nutrition content for future schedules.
      </p>

      <div className="mt-8 max-w-3xl rounded-xl border p-6">
        <h2 className="text-xl font-semibold">Create Nutrition Template</h2>
        ...
        <div className="mt-4 space-y-4">
          ...
          <textarea
            ...
            placeholder="Nutrition content / menu details"
            value={form.content}
          ...
          >
            {saving ? "Saving..." : "Create Nutrition Template"}
          </button>
        </div>
      </div>
