...
export const sendPushNotification = onCall({ region: "us-central1" }, async (request) => {
  ...
  if (audience === "selected" && selectedUserIds.length === 0) {
    throw new HttpsError("invalid-argument", "selectedUserIds is required for selected audience.");
  }

  const tokenDocs = await db.collectionGroup("pushTokens").where("enabled", "==", true).get();
  ...
  response.responses.forEach((result, index) => {
    if (!result.success && result.error && invalidTokenErrors.has(result.error.code)) {
      invalidTokens.push(uniqueTokens[index]);
    }
  });

  if (invalidTokens.length > 0) {
    const removals = tokenDocs.docs.filter((snap) => invalidTokens.includes(snap.get("token")));
